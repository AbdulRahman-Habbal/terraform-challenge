...
# We use 'HTMLEOF' in quotes to prevent the shell from evaluating variables here
...
<body>
  <div class="container">
     <h1>Two-Tier App &mdash; Abdul Rahman Habbal</h1>
...
# We DO NOT quote 'NGINXEOF' so that Terraform can inject the ${backend_dns} variable
...
server {
    ...
    location /api/ {
        proxy_pass http://${backend_dns}:80;
...
# Remove default configs and start service
